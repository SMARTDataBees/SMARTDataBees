...
namespace Carbon.AutoUpdate.Common
{
	...
	public class AutoUpdateDownloader
	{
			...
			AutoUpdateDownloadDescriptor downloadDescriptor)
		{
			...
			var myTraceCategory = $"'{GetType().Name}'";

			try
			{
				// set the progress to zero for the start
				SetDownloadProgress(progressViewer, 0, downloadDescriptor.Manifest.SizeOfUpdate);
				...
				Debug.WriteLine(
				    $"Preparing to download update.\n\tThe update will be downloaded from '{downloadDescriptor.Manifest.UrlOfUpdate}'.\n\tThe update will be downloaded to '{downloadDescriptor.DownloadedPath}'.", myTraceCategory);
				...
				Debug.WriteLine(
				    $"Preparing to download update.\n\tOpening stream to the remote url '{downloadDescriptor.Manifest.UrlOfUpdate}'.", myTraceCategory);
				...
				Debug.WriteLine(
				    $"Preparing to download update.\n\tOpening stream to the local url '{downloadDescriptor.DownloadedPath}'.", myTraceCategory);
				...
				if (remoteStream != null)
				{
					...
					while (true)
					{
						...
						var bytesToReceive = downloadDescriptor.Manifest.SizeOfUpdate - bytesDownloaded;

						// correct it if it's more than the segment size
						if (bytesToReceive > _segmentSize)
							bytesToReceive = _segmentSize;

						var segment = new byte[bytesToReceive];

						// read a segment off the socket
						var bytesReceived = remoteStream.Read(segment, 0, (int)bytesToReceive);
						...
						SetDownloadProgress(progressViewer, bytesDownloaded, downloadDescriptor.Manifest.SizeOfUpdate);
					}
				}

				Debug.WriteLine($"The update was successfully downloaded to '{downloadDescriptor.DownloadedPath}'.", myTraceCategory);
		...
		public virtual string FormatFileLengthForDisplay(long dwFileSize)
		{
			...
			var fileSizeInt = Convert.ToInt32(fileSize);
		...
		{								
			var p = (bytesReceived / (double)bytesTotal) * 100;
			var percent = (int)p;
			
			var title = "AutoUpdate Progress..." + $"({percent}% Completed)";
			var received = FormatFileLengthForDisplay(bytesReceived);
			var total = FormatFileLengthForDisplay(bytesTotal);
			var description = $"Progress: ({received} of {total} downloaded)";
